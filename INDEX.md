# MFL Workspace — Repo Index

> Catalog of all repos in the MFL workspace ecosystem.

| Repo | Description | Access | Key Skills |
|------|-------------|--------|------------|
| **mfl-workspace** | Umbrella repo — shared context, setup, universal skills | All employees | `/commit`, `/project-create` |
| **mfl-wiki** | Knowledge system — KB, memory, sources | All employees | `/wiki-ingest`, `/wiki-recap`, `/wiki-update`, `/source-import` |
| **mfl-confidential** | Finance, legal, projections | Mathurin, Yann, CFO | — |
| **mfl-creators** | Creator CRM — profiles, outreach, contracts, audits | Creator/marketing team | `/creator-digest`, `/creator-update`, `/creator-outreach`, `/creator-audit`, `/creator-contract`, `/creator-activate`, `/creator-discover`, `/creator-payments`, `/discord-read-dm`, `/email` |
| **mfl-marketing** | Marketing projects — campaigns, promos, acquisition | Marketing team | `/dapper-credit`, `/dapper-credit-inbox`, `/meta-ads` |
| **mfl-product** | Product projects — features, discovery, UX | Product team | — |
| **mfl-tech** | Tech projects — migrations, infra, integrations | Tech team | — |
| **mfl-data** | Data projects — analytics, cohorts, KPIs | Data team | `/data-cohort-update` |

## How to get started

```bash
git clone https://github.com/Metaverse-Football-League/mfl-workspace.git ~/mfl-workspace
cd ~/mfl-workspace
./setup.sh
```

Then navigate to your domain's repo and start working:

```bash
cd mfl-marketing && claude
```
